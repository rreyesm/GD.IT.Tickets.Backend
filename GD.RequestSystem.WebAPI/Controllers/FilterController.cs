...
namespace GD.RequestSystem.WebAPI.Controllers
{
    ...
    public class FilterController : ControllerBase
    {
        ...
        [HttpGet]
        public async Task<ResultModel<List<Petition>>> GetPetitionByUserId()
        {
            try
            {
                var userID = int.Parse(User.FindFirst(ClaimTypes.NameIdentifier)?.Value);
                var result = await filterService.GetPetitionByUserId(userID);
                if (result.Response == false)
                {
                    return new ResultModel<List<Petition>>
                    {
                        Response = false,
                        Data = null,
                        Message = result.Message
        ...
        [HttpGet]
        public async Task<ResultModel<List<Petition>>> GetAllPetitionByUserAsigned()
        {
            try
            {
                var userID = int.Parse(User.FindFirst(ClaimTypes.NameIdentifier)?.Value);
                var result = await filterService.GetAllPetitionByUserAsigned(userID);
                if (result.Response == false)
                {
                    return new ResultModel<List<Petition>>
                    {
